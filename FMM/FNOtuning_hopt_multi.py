...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2,  width, num_spectral_layers=4, mlp_hidden_dim=128, mlp_LN=False, activation='gelu', mode_threshold=False, kernel_type='p', padding=9):
        ...
        self.Conv2d_list = nn.ModuleList([nn.Conv2d(self.width, self.width, 1)])
        if kernel_type == 'p':
...
#         x = torch.cat((x, grid), dim=-1)
        ...
        for i in range(self.num_spectral_layers - 1):
...
def objective(modes, width, learning_rate=0.001, weight_decay=1e-4, batch_size=20, num_spectral_layers=4, activation='gelu', mlp_hidden_dim=128, optimizer_type='Adam', show_conv=False, mode_threshold=False, kernel_type='p', loss_type='h1', sampling_rate=2,  epochs=100, padding=15):
    ...
    model = FNO2d(modes, modes, width, num_spectral_layers=num_spectral_layers, mlp_hidden_dim=mlp_hidden_dim, mode_threshold=mode_threshold, kernel_type=kernel_type, padding=padding).to(device)
...
#     if loss_type == 'h1':        
#         myloss = HsLoss(d=2, p=2, k=1, size_average=False, a=[2.,])
#         myloss.cuda(device)
#         valoss = LpLoss(size_average=False)
#     elif loss_type == 'l2':
#         valoss = HsLoss(d=2, p=2, k=1, size_average=False)
#         myloss = LpLoss(size_average=False)
#     elif loss_type == 'h2':
#         myloss = HsLoss(d=2, p=2, k=2, size_average=False)
#         valoss = LpLoss(size_average=False)
#     else: raise NameError('invalid loss type')
